...
class Upsample(nn.Module):
    ...
    def forward(self, inp):
        ret_val = F.conv_transpose2d(self.pad(
            inp), self.filt, stride=self.stride, padding=1 + self.pad_size, groups=inp.shape[1])[:, :, 1:, 1:]
        if (self.filt_odd):
            return ret_val
        else:
            return ret_val[:, :, :-1, :-1]
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
    ...
    elif netG == 'resnet_9blocks_nice_dcl':
        net = ResnetGeneratorNICEDCL(
            input_nc, output_nc, ngf, n_blocks=6)
        ...
        n_blocks = 8
        net = G_Resnet(input_nc, output_nc, opt.nz, num_downs=2,
                       n_res=n_blocks - 4, ngf=ngf, norm='inst', nl_layer='relu')
    else:
        raise NotImplementedError(
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        net = DiscriminatorNICEDCL(
            input_nc, ndf, n_layers=7)
    elif netD == 'n_layers':  # more options
        net = NLayerDiscriminator(
            input_nc, ndf, n_layers_D, norm_layer=norm_layer, no_antialias=no_antialias,)
    ...
    def __call__(self, prediction, target_is_real):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            if target_is_real:
                ...
                minvalue = torch.min(-prediction - 1,
                                     torch.zeros(prediction.shape).to(prediction.device))
                loss = -torch.mean(minvalue)
        return loss
...
class StridedConvF(nn.Module):
    ...
    def update_moving_average(self, key, x):
        ...
        self.moving_averages[key] = self.moving_averages[key] * \
            0.999 + x.detach() * 0.001
    ...
    def __init__(self, input_nc, output_nc, nz, num_downs, n_res, ngf=64,
        ...
        if nz == 0:
            self.dec = Decoder(n_downsample, n_res, self.enc_content.output_dim,
                               output_nc, norm=norm, activ=nl_layer, pad_type=pad_type, nz=nz)
        else:
            self.dec = Decoder_all(n_downsample, n_res, self.enc_content.output_dim,
                                   output_nc, norm=norm, activ=nl_layer, pad_type=pad_type, nz=nz)
...
class StyleEncoder(nn.Module):
    def __init__(self, n_downsample, input_dim, dim, style_dim, norm, activ, vae=False):
        ...
        self.model += [Conv2dBlock(input_dim, dim, 7, 1, 3,
                                   norm=norm, activation=activ, pad_type='reflect')]
        for i in range(2):
...
class Decoder_all(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, norm='batch', activ='relu', pad_type='zero', nz=0):
        ...
        self.resnet_block = ResBlocks(
            n_res, dim, norm, activ, pad_type=pad_type, nz=nz)
        self.n_blocks = 0
        # upsampling blocks
        for i in range(n_upsample):
            block = [Upsample2(scale_factor=2), Conv2dBlock(
                dim + nz, dim // 2, 5, 1, 2, norm='ln', activation=activ, pad_type='reflect')]
            setattr(self, 'block_{:d}'.format(
                self.n_blocks), nn.Sequential(*block))
            self.n_blocks += 1
            dim //= 2
        # use reflection padding in the last conv layer
        setattr(self, 'block_{:d}'.format(self.n_blocks), Conv2dBlock(
            dim + nz, output_dim, 7, 1, 3, norm='none', activation='tanh', pad_type='reflect'))
        self.n_blocks += 1
...
class Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, norm='batch', activ='relu', pad_type='zero', nz=0):
        ...
        self.model = []
        # AdaIN residual blocks
        self.model += [ResBlocks(n_res, dim, norm, activ,
        ...
        for i in range(n_upsample):
            if i == 0:
                ...
                input_dim = dim
            self.model += [Upsample2(scale_factor=2), Conv2dBlock(
                input_dim, dim // 2, 5, 1, 2, norm='ln', activation=activ, pad_type='reflect')]
            dim //= 2
        # use reflection padding in the last conv layer
        self.model += [Conv2dBlock(dim, output_dim, 7, 1, 3,
                                   norm='none', activation='tanh', pad_type='reflect')]
        self.model = nn.Sequential(*self.model)
    ...
    def __init__(self, num_blocks, dim, norm='inst', activation='relu', pad_type='zero', nz=0):
        ...
        for i in range(num_blocks):
            self.model += [ResBlock(dim, norm=norm,
                                    activation=activation, pad_type=pad_type, nz=nz)]
        self.model = nn.Sequential(*self.model)
...
class ResBlock(nn.Module):
    def __init__(self, dim, norm='inst', activation='relu', pad_type='zero', nz=0):
        ...
        model += [Conv2dBlock(dim, dim + nz, 3, 1, 1,
...
class PatchDiscriminator(NLayerDiscriminator):
    ...
    def forward(self, input):
        ...
        input = input.permute(0, 2, 4, 1, 3, 5).contiguous().view(
            B * Y * X, C, size, size)
...
class GroupedChannelNorm(nn.Module):
    ...
    def forward(self, x):
        shape = list(x.shape)
        new_shape = [shape[0], self.num_groups,
                     shape[1] // self.num_groups] + shape[2:]
        x = x.view(*new_shape)
...
class ResnetGeneratorNICEDCL(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, img_size=256, light=False):
        assert (n_blocks >= 0)
        super(ResnetGeneratorNICEDCL, self).__init__()
        ...
        for i in range(n_blocks):
            setattr(self, 'UpBlock1_' + str(i+1),
                    ResnetAdaILNBlockNICEDCL(ngf * mult, use_bias=False))
    ...
    def forward(self, input):
        ...
        fake = self.UpBlock2(x)

        return fake


class ResnetBlockNICEDCL(nn.Module):
    def __init__(self, dim, use_bias):
        super(ResnetBlockNICEDCL, self).__init__()
        conv_block = []
        conv_block += [nn.ReflectionPad2d(1),
...
class ResnetAdaILNBlockNICEDCL(nn.Module):
    def __init__(self, dim, use_bias):
        super(ResnetAdaILNBlockNICEDCL, self).__init__()
        self.pad1 = nn.ReflectionPad2d(1)
        self.conv1 = nn.Conv2d(dim, dim, kernel_size=3,
                               stride=1, padding=0, bias=use_bias)
        self.norm1 = adaILNNICEDCL(dim)
        self.relu1 = nn.ReLU(True)

        self.pad2 = nn.ReflectionPad2d(1)
        self.conv2 = nn.Conv2d(dim, dim, kernel_size=3,
                               stride=1, padding=0, bias=use_bias)
        self.norm2 = adaILNNICEDCL(dim)
...
class ILNNICEDCL(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.9, using_moving_average=True, using_bn=False):
        super(ILNNICEDCL, self).__init__()
        self.eps = eps
        self.momentum = momentum
        self.using_moving_average = using_moving_average
        self.using_bn = using_bn
...
class DiscriminatorNICEDCL(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=7):
        super(DiscriminatorNICEDCL, self).__init__()
        ...
        self.fc = nn.utils.spectral_norm(
            nn.Linear(ndf * mult * 2, 1, bias=False))
        ...
        mult = 2 ** (n_layers - 4 - 1)
        Dis0_1 = [nn.ReflectionPad2d(1), nn.utils.spectral_norm(nn.Conv2d(
            ndf * mult, ndf * mult * 2, kernel_size=4, stride=1, padding=0, bias=True)), nn.LeakyReLU(0.2, True)]
        mult = 2 ** (n_layers - 4)
